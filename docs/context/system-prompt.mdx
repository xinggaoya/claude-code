...
当前 `openai` provider 采用双路径：

- **官方 OpenAI 模型** 默认优先走 **Responses API**
- **OpenAI-compatible 网关** 默认继续走 **Chat Completions API**
...
- `OPENAI_REASONING_EFFORT` — 给 OpenAI reasoning 模型显式设置 `reasoning_effort`（如 `low` / `high` / `xhigh`）
- `OPENAI_USE_RESPONSES=1` — 强制使用 Responses API
- `OPENAI_USE_CHAT_COMPLETIONS=1` — 强制使用 Chat Completions API
